...
### Fixed
- Run `govulncheck` from a pinned `go install golang.org/x/vuln/cmd/govulncheck@v1.8.0` and the repository's own Go version instead of `golang/govulncheck-action@v1`. The action always installed `@latest` under `GOTOOLCHAIN=local`, so any upstream release that raised the minimum Go version broke the install and took the gate down on every branch until someone noticed. The scanner version now moves only when this file says it does, and the vulnerability database is still fetched at scan time, so new advisories are still reported. Bumping the pin to a release that needs a newer Go fails the install until `go.mod` moves too, which is the intended signal rather than a silent toolchain download.
