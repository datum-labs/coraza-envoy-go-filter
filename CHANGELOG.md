...
### Fixed
- Run `govulncheck` from a nested `tools/go.mod` that declares it with a Go `tool` directive, instead of `golang/govulncheck-action@v1`. The action always installed `@latest` under `GOTOOLCHAIN=local`, so any upstream release that raised the minimum Go version broke the install and took the gate down on every branch until someone noticed. The scanner version is now pinned and checksummed in `tools/go.sum` and kept out of the filter's own module graph, so a scanner upgrade never changes the dependencies that ship in the shared object. Renovate tracks the pin natively once it processes this repository, and the vulnerability database is still fetched at scan time, so new advisories are still reported. A scanner release that needs a newer Go fails the scan under `GOTOOLCHAIN=local` until `go.mod` moves too, which is the intended signal rather than a silent toolchain download.
...
- Document commit-message, branch-naming (`<type>/<short-name>`), and parallel-worktree conventions in `CONTRIBUTING.md`.
