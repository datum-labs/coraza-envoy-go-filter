...
# Copy the plugin source (root main.go + internal/ packages).
# The alpha4 lineage uses internal/ packages rather than a src/ directory.
COPY main.go ./
...
# Build tags match the alpha4 Makefile: coraza.rule.multiphase_evaluation,memoize_builders
RUN if [ "${BUILDPLATFORM}" = "${TARGETPLATFORM}" ]; then \
      CC_COMPILER="gcc"; \
    elif [ "${TARGETARCH}" = "arm64" ]; then \
